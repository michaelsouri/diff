...
void version(void) {
  ...
  printf("Written by Michael Souri\n\n");
}
...
void init_options_files(int argc, const char* argv[]) {
  ...
  while (argc-- > 0) {
    ...
    if (arg[0] != '-') {
      if (cnt == 2) {
        fprintf(stderr, "v1.0 - 2 FILES MAX\n");
  ...
  if (report_identical && !different) { printf("FILES ARE IDENTICAL.\n\n");   exit(0); }

  if (showbrief && different) { printf("FILES ARE DIFFERENT.\n\n");   exit(0); }
}
